...
- 추억 모아보기(screen 5) 탭 삭제 = 사용자가 명시적으로 요청해 `/memories` 라우트와 하단탭/사이드바 메뉴 항목을 완전히 제거함(2026-07-04) / photos 테이블·업로드·캘린더 썸네일·날짜상세 사진 첨부 기능은 그대로 유지(추억 탭은 그 데이터를 모아보는 화면일 뿐이라 기반 기능은 안 건드림) / 되돌리려면 git 이력에서 `src/app/(app)/memories/` 디렉터리와 `nav-items.ts`의 항목을 복원
